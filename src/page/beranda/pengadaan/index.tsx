...
import ReportPengadaanPdf from "./print";
...
const PengadaanBuku = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axiosServices().get(
        `/api/v1/unibookstore/pengadaan/list`,
        {
          params: {
            page: page,
            "per-page": rows,
      ...
      return response.data.data_pengadaan;
    } catch (error: any) {
      if (error.response) {
  ...
  const loadDataRincian = async () => {
    toastRef.current?.show({
      severity: "info",
      summary: "Info",
      detail: "Mohon tunggu, data sedang di proses.",
      life: 1000,
    });
    try {
      const response = await axiosServices().get(
        "/api/v1/unibookstore/pengadaan/report_pengadaan"
      );

      setDataLaporanPrint(response.data.data_pengadaan);
  ...
  return (
    <>
        ...
        <div className="flex flex-col justify-center gap-3">
          <div className="grid base-card bg-background gap-2 bg-white">
            <div className="flex justify-between items-center p-2">
              <div className="font-semibold text-xl">Data Pengadaan Buku</div>
              <Button
                type="button"
                label="Kembali"
            ...
            <div className="grid base-card-with-title p-2 gap-4">
              <div className="flex flex-col md:flex-row items-stretch md:items-center gap-3 w-full">
                <InputText
                  ...
                  placeholder="Cari data pengadaan buku"
                  onKeyDown={(event) => {
                    if (event.key === "Enter") {
                      refetchListBuku();
                    }
                ...
                <div className="flex gap-2 flex-col md:flex-row w-full md:w-auto">
                  <Button
                    type="button"
                    label="Reset"
                    className="w-full md:w-auto p-button-sm text-xs"
                    icon="pi pi-refresh"
                    severity="danger"
                      ...
                      <td colSpan={4} className="text-center py-10">
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <span className="text-sm">
                            <b>Belum ada data Pengadaan Buku.</b> <br />
                            Silakan menambahkan data Pengadaan Buku.
                          </span>
                        </div>
                      </td>
                    </tr>
      ...
      <div hidden>
        <ReportPengadaanPdf
          ref={contentRef}
          dataLaporanPrint={dataLaporanPrint}
        />
      </div>
    </>
  );
