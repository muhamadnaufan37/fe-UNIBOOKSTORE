...
const KelolaDataBuku = () => {
  ...
  const [balikanDataPenerbit, setBalikanDataPenebit] = useState([]);
  const [balikanDataKategori, setBalikanDataKategori] = useState([]);
  ...
  const fetchDataPenerbit = async () => {
    try {
      const response = await axiosServices().get(
        `/api/v1/unibookstore/penerbit/list_data_penerbit`
      );

      setBalikanDataPenebit(
        response.data.data_penerbit.map((item: any) => ({
          value: item.id,
          label: item.nama,
        }))
      );
  ...
  const fetchDataKategori = async () => {
    try {
      const response = await axiosServices().get(
        `/api/v1/unibookstore/buku/kategori/list_data_kategori`
      );

      setBalikanDataKategori(
        response.data.data_kategori.map((item: any) => ({
          value: item.id,
  ...
  const DetailDataFetch = async (id: any, visibilityOption: any) => {
    try {
      if (!isDataFetching) {
        ...
        const bukuDataArray = response.data.data_buku;
        setUserData(bukuDataArray);

        switch (visibilityOption) {
          case 2:
            navigate("/admin/kelola-buku/detail-buku", {
              state: {
                detailData: bukuDataArray,
                balikanDataPenerbit: balikanDataPenerbit,
                balikanDataKategori: balikanDataKategori,
              },
              replace: true,
            });
            break;
          case 3:
            navigate("/admin/kelola-buku/update-buku", {
              state: {
                detailData: bukuDataArray,
                balikanDataPenerbit: balikanDataPenerbit,
                balikanDataKategori: balikanDataKategori,
  ...
  const onDeleteData = async () => {
    try {
      const response = await axiosServices().delete(
        `/api/v1/unibookstore/buku/delete`,
        {
          data: { id: userData?.id },
        }
      );
        ...
        <div className="flex flex-col justify-center gap-3">
          <div className="grid base-card bg-background gap-2 bg-white">
            <div className="flex justify-between items-center p-2">
              <div className="font-semibold text-xl">Kelola Data Buku</div>
              <Button
                type="button"
                label="Kembali"
                className="p-button-sm text-xs"
            ...
            <div className="grid base-card-with-title p-2 gap-4">
              <div className="flex items-center gap-3 md:flex-row flex-col w-full">
                ...
                <Button
                  type="button"
                  label="Reset"
                  className="min-w-[9rem] p-button-sm text-xs"
                  icon="pi pi-refresh"
                  severity="danger"
                ...
                <Button
                  ...
                  severity="info"
                  onClick={() =>
                    navigate("/admin/kelola-buku/add-buku", {
                      state: {
                        balikanDataPenerbit: balikanDataPenerbit,
                        balikanDataKategori: balikanDataKategori,
                      },
